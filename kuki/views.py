...
def dictionary(request):
    ...
    try:
        page = request.GET.get('word', 1)

        current_directory = os.path.dirname(os.path.abspath(__file__))

        file_path = os.path.join(current_directory, "pickledatabase.dat")

        with open(file_path, "rb") as file:
            malayalam_dict = pickle.load(file)
    ...
    if word in malayalam_dict:
        meanings = malayalam_dict[word].split(';')
    ...
    context={"detail":"hai"}
    return JsonResponse({'word': word, 'meanings': meanings})
